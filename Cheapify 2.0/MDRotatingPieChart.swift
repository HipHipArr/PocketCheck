...
struct Properties {
    ...
    var smallRadius:CGFloat = 80//50
    //biggest of both radius
    var bigRadius:CGFloat = 140//120
    ...
    var fontTextCenter: UIFont = UIFont(name: "STHeitiTC-Light", size: 2)!
...
class MDRotatingPieChart: UIControl {
    ...
    //label in the center (displays the text)
    var labelCenter:UILabel = UILabel()
    
    //below labelCenter, displays the sum of entries in that category (in dollars)
    var labelCenterNumber:UILabel = UILabel()
    ...
    override init(frame: CGRect) {
        ...
        pieChartCenter.y = frame.height/2+frame.height/5-6
        
        //builds and adds the centered label
        labelCenter.frame = CGRectZero
        labelCenter.center = CGPointMake(pieChartCenter.x, pieChartCenter.y - 20)
        labelCenter.textColor = UIColor.blackColor()
        labelCenter.textAlignment = NSTextAlignment.Center
        labelCenter.font = UIFont(name: "Arial", size: 20)!
        labelCenter.hidden = true
        addSubview(labelCenter)
        labelCenterNumber.frame = CGRectZero
        labelCenterNumber.center = CGPointMake(pieChartCenter.x, pieChartCenter.y + 20)
        labelCenterNumber.textColor = UIColor.blackColor()
        labelCenterNumber.textAlignment = NSTextAlignment.Center
        labelCenterNumber.font = UIFont(name: "Arial", size: 30)!
        addSubview(labelCenterNumber)
    }
    ...
    */
    func reset() {
        ...
        labelCenter.text = ""
        labelCenterNumber.transform = self.transform
        labelCenterNumber.text = ""
    ...
    */
    func prepareSlice(inout angleSum:CGFloat, inout currentStartAngle:CGFloat, total:CGFloat, index:Int) {
        ...
        let currentLabelNum = datasource.labelNumForSliceAtIndex(index)
        
        //create slice
        let slice = createSlice(currentStartAngle, end: CGFloat(currentStartAngle - currentAngle), color:currentColor, label:currentLabel, labelnum: currentLabelNum, value:currentValue, percent:100 * currentValue/total)
    ...
    */
    func openSlice(index:Int) {
        ...
        labelCenterNumber.text = datasource.labelNumForSliceAtIndex(index)//formatFromDisplayValueType(slicesArray[index], displayType: properties.displayValueTypeCenter)
        let centerTmpNumber = labelCenterNumber.center
        labelCenterNumber.sizeToFit()
        labelCenterNumber.center = centerTmpNumber
        
        labelCenter.hidden = false
        labelCenterNumber.hidden = false
        var cpt = 0;
        ...
        var cptnum = 0;
        for (; cptnum < datasource?.numberOfSlices(); ++cptnum) {
            if(!frameFitInPath(labelCenterNumber.frame, path: slicesArray[cptnum].paths.bezierPath, inside:false)) {
                //labelCenterNumber.hidden = true
                break;
            }
        }
    ...
    */
    ...
    override func continueTrackingWithTouch(touch: UITouch, withEvent event: UIEvent) -> Bool {
        ...
        //labelCenterNumber.transform = CGAffineTransformRotate(savedTransformCenter, angleDifference)
    ...
    */
    ...
    :returns: a new slice
    */
    func createSlice(start:CGFloat, end:CGFloat, color:UIColor, label:String, labelnum: String, value:CGFloat, percent:CGFloat) -> Slice {
        ...
        var slice = Slice(myPaths: path, myShapeLayer: mask, myAngle: end-start, myLabel:label, myLabelNum: labelnum, myValue:value, myPercent:percent)
...
struct Slice {
    ...
    init(myPaths:DualPath, myShapeLayer:CAShapeLayer, myAngle:CGFloat, myLabel:String, myLabelNum: String, myValue:CGFloat, myPercent:CGFloat) {
        self.paths = myPaths
        self.shapeLayer = myShapeLayer
        self.angle = myAngle
        self.label = myLabel
        self.labelnum = myLabelNum
        self.value = myValue
