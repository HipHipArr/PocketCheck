...
//  Cheapify 2.0
//
//  Created by Carol Wang on 7/6/15.
//  Copyright (c) 2015 Hip Hip Array[]. All rights reserved.
//

import UIKit

class CategoryEntries: UIViewController, UITableViewDelegate, UITableViewDataSource{
    ...
    func setup(){
        ...
        if(entryMgr.entries.count>0){
            for index in 0...entryMgr.entries.count-1 {
                if((entryMgr.entries[index].valueForKey("category") as? String)! == entryMgr.categoryname){
                    entryMgr.tempArr.append(entryMgr.entries[index])
                }
            }
        }
        var imageView = UIImageView(frame: CGRectMake(0, -22, tblTasks.frame.width, tblTasks.frame.height+98))
        let image = chooseImage(entryMgr.categoryname)
        imageView.image = image
        self.view.addSubview(imageView)
        chooseColor(entryMgr.categoryname)
        self.view.bringSubviewToFront(tblTasks)
